...
func TestParseCgroupString(t *testing.T) {
   ...
   1:name=systemd:/system.slice/system-sshd.slice/sshd@26-10.240.0.15:22-xxx.yyy.zzz.aaa:33678.service`,
      ...
      },
      {
         ...
         expectedError: fmt.Errorf(`invalid cgroup entry: must contain atleast two colons: malformed input`),
      },
   }
   ...
   for ndx, testCase := range testCases {
      ...
      if err != nil {
         if testCase.expectedError == nil || testCase.expectedError.Error() != err.Error() {
            t.Errorf("%v: expected error %v, got error %v", ndx, testCase.expectedOutput, out)
         }
      } else {
         if !reflect.DeepEqual(testCase.expectedOutput, out) {
            t.Errorf("%v: expected output %v", ndx, testCase.expectedOutput, out)
         }
      }
   }
}
